...
const Lab1Result = props => {
  ...
  return (
    <div>
      ...
      <div>
        <h4 className="mb-3 display-4">Степени вершины:</h4>
        <ul>{Degrees}</ul>
        <p>Изолированные вершины: {isolated}</p>
        {!props.isOrGraph && <p>Висячие вершины: {leaf}</p>}
      </div>
      <div style={{display: 'flex', 'justify-content': 'space-around', 'margin-bottom': '3rem'}}>
        <div>
          <h4 className="mb-3 display-4">С матрица(n-1):</h4>
          <Matrix matrix={Cmatrix} />
        </div>
        <div>
          <h4 className="mb-3 display-4">С матрица(n):</h4>
          <Matrix matrix={Cmatrix2} />
        </div>
      </div>
      <div>
        <h4 className="mb-3 display-4">Количество маршрутов заданной длины:</h4>
        <label>Длина :</label>
